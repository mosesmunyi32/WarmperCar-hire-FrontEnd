...
export default function CustomerDashboard() {
  ...
  useEffect(() => {
    bookingService
      ...
      .then((bs) => {
        setBookings(bs);
        // Find inspections still awaiting the customer's confirmation.
        ...
        Promise.allSettled(
          active.map((b) =>
            inspectionService.getBookingInspectionsForCustomer(b.id),
          ),
        ).then((results) => {
          ...
          results.forEach((res, i) => {
            ...
            const b = active[i];
            res.value.forEach((insp) => {
              if (insp.customerResponse === "PENDING") {
                actions.push({
                  bookingId: b.id,
                  label:
                    insp.inspectionType === "PRE_INSPECTION"
                      ? "Confirm your pre-inspection"
                      : "Confirm your post-inspection",
                  sub:
  ...
  return (
    <CustomerLayout breadcrumbs={[{ label: "Dashboard" }]}>
      <div className="flex flex-col gap-6 p-6">
        ...
        {/* ── Action Required — pending inspection confirmations ── */}
        {pendingActions.length > 0 && (
          <div className="rounded-2xl border border-gold/35 bg-gold/[0.06] overflow-hidden">
