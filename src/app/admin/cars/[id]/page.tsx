...
export default function EditCarPage() {
  ...
  const [damageResolved, setDamageResolved] = useState(() => {
    try {
      return localStorage.getItem(`damage_resolved_${id}`) === "true";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    Promise.allSettled([
      carService.getAdminCarById(id).then((c) => {
        setCar(c);
        setForm(carToForm(c));
      }),
      ...
      bookingService
        ...
        .then((all: AdminBooking[]) => {
          ...
          all
            .filter((b) => b.carId === id)
            .forEach((b) => map.set(b.bookingReference, b.id));
          setRefToId(map);
        })
        .catch(() => {}),
    ]).finally(() => {
  ...
  const hasDamageRecord =
    carBookings.some((b) => b.carWasDamaged) && !damageResolved;

  const handleResolveDamage = () => {
    try {
      localStorage.setItem(`damage_resolved_${id}`, "true");
    } catch {}
    ...
    bookingService
      ...
      .then((bookings) => {
        const pending = bookings.find(
          (b) => b.carId === id && b.bookingStatus === "PENDING",
        );
  ...
  const handleToggleAvailability = async () => {
    ...
    try {
      ...
      toast.success(
        result.available
          ? "Car marked as available for booking."
  ...
  {
    existingBooking && (
      <div className="mx-5 mt-4 rounded-lg border border-gold/30 bg-gold/10 px-4 py-3">
        <p className="font-semibold text-navy text-sm">
          You&apos;ve already booked this car
        </p>
...
function getDayBookingStatus(
  date: Date,
  bookings: BookingHistoryResponse[],
  today: Date,
): BookingDayStatus | null {
  const priority: BookingDayStatus[] = [
    "active",
    "upcoming-confirmed",
    "upcoming-pending",
    "completed",
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-xl border border-light-gray px-4 py-3">
          ...
          <p className="text-xl font-bold text-navy">
            KES {monthRevenue.toLocaleString()}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-light-gray px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          {!selectedDay ? (
            <div className="bg-white rounded-xl border border-light-gray p-8 text-center text-muted-foreground h-full flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-xs">
                Click any highlighted date on the calendar to see booking
                details
              </p>
            </div>
          ) : (
                ...
                <div className="divide-y divide-light-gray">
                  {selectedDayBookings.map((b) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        const bid = refToId.get(b.bookingReference);
                        if (bid) router.push(`/admin/bookings/${bid}`);
                      }}
...
function CarBookingsTab({
    ...
    return (
      <div className="space-y-3">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="bg-white rounded-xl border border-light-gray p-5 animate-pulse h-20"
          />
        ))}
      </div>
    );
  ...
  const Section = ({
    ...
    items.length === 0 ? null : (
      <div className="mb-6">
        <h3
          className={`flex items-center gap-2 text-sm font-semibold mb-3 ${accent}`}
        >
          {icon} {title}{" "}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {(() => {
                      try {
                        return format(new Date(b.startDate), "MMM d, yyyy");
                      } catch {
                        return b.startDate;
                      }
                    })()}
                    ...
                    {(() => {
                      try {
                        return format(new Date(b.endDate), "MMM d, yyyy");
                      } catch {
                        return b.endDate;
                      }
                    })()}
