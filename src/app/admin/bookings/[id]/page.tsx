...
function AdminInspectionTabs({
  ...
  const isFormForActiveTab =
    (activeTab === "PRE" &&
      (inspectionForm === "create-pre" ||
        (inspectionForm === "edit" &&
          editingInspection?.inspectionType === "PRE_INSPECTION"))) ||
    (activeTab === "POST" &&
      (inspectionForm === "create-post" ||
        (inspectionForm === "edit" &&
          editingInspection?.inspectionType === "POST_INSPECTION")));

  return (
    <div
      id="action-inspection"
      className="bg-white rounded-xl border border-light-gray shadow-sm overflow-hidden mb-4"
    >
      {/* Header */}
      ...
      <div className="flex border-b border-light-gray px-5">
        {(["PRE", "POST"] as const).map((tab) => {
          const isPre = tab === "PRE";
          const hasDot = isPre
            ? pendingPre || confirmedPre
            : pendingPost || confirmedPost;
        ...
        {inspectionsLoading ? (
          ...
          <>
            <AdminInspectionCard
              ...
              onEdit={() => {
                setEditingInspection(activeInspection);
                setInspectionForm("edit");
              }}
              onConfirmOnBehalf={() =>
                handleConfirmOnBehalf(activeInspection.id)
              }
                  ...
                  initial={
                    editingInspection
                      ? {
                          ...
                          inspectionComment:
                            editingInspection.inspectionComment ?? undefined,
                          damagedPhotos: editingInspection.damagedPhotos,
                          isDamageChargeRequired:
                            editingInspection.isDamageChargeRequired ??
                            undefined,
                          damageChargeAmount:
                            editingInspection.damageChargeAmount ?? undefined,
                        }
                      : undefined
                  }
...
function AdminBookingDetailContent() {
  ...
  const [inspectionForm, setInspectionForm] = useState<
    "create-pre" | "create-post" | "edit" | null
  >(null);
  const [editingInspection, setEditingInspection] =
    useState<AdminInspectionResponse | null>(null);
  const [submittingInspection, setSubmittingInspection] = useState(false);
  const [confirmingOnBehalfId, setConfirmingOnBehalfId] = useState<
    string | null
  >(null);
  ...
  const preInspection = inspections.find(
    (i) => i.inspectionType === "PRE_INSPECTION",
  );
  const postInspection = inspections.find(
    (i) => i.inspectionType === "POST_INSPECTION",
  );

  // Deep-link from the dashboard "Action Required" feed: scroll to the relevant
  // section (and open the pre-inspection form) based on ?action=...
  useEffect(() => {
    if (actionHandledRef.current) return;
    if (!booking || inspectionsLoading || !action) return;
    ...
    if (action === "approve") {
      scrollTo("action-approve");
    } else if (action === "pre-inspection") {
      if (!preInspection) setInspectionForm("create-pre");
      scrollTo("action-inspection");
    } else if (action === "post-inspection") {
      scrollTo("action-inspection");
    } else if (action === "mileage-start") {
      scrollTo("action-mileage");
    }
  }, [booking, inspectionsLoading, action, preInspection]);
    ...
    try {
      if (inspectionForm === "edit" && editingInspection) {
        const updated = await inspectionService.updateInspection(
          ...
          data as UpdateInspectionRequest,
        );
        setInspections((prev) =>
        ...
        const created =
          inspectionForm === "create-post"
            ? await inspectionService.createPostInspection(
                data as CreateInspectionRequest,
              )
            : await inspectionService.createPreInspection(
                data as CreateInspectionRequest,
              );
        setInspections((prev) => [...prev, created]);
        toast.success(
          inspectionForm === "create-post"
            ? "Post-inspection created."
            : "Pre-inspection created.",
        );
        ...
        if (inspectionForm === "create-post") {
          ...
          if (postData.isDamageChargeRequired && car?.isAvailable) {
            try {
              ...
              setCar((prev) =>
                prev ? { ...prev, isAvailable: result.available } : prev,
              );
              toast.info(
                "Car automatically marked as unavailable due to damage charge.",
              );
            } catch {}
          }
        }
  ...
  const handleConfirmOnBehalf = async (inspectionId: string) => {
    ...
    try {
      await inspectionService.confirmInspectionForcustomer(booking.id);
      setInspections((prev) =>
        prev.map((i) =>
          i.id === inspectionId
            ? { ...i, customerResponse: "CONFIRMED" as const }
            : i,
        ),
      );
  ...
  const handleExtensionDecision = async (
    ...
    try {
      ...
      setExtensions((prev) =>
        prev.map((e) => (e.id === extensionId ? updated : e)),
      );
  ...
  const handleAdminCancel = async () => {
    ...
    setCancelling(true);
    try {
      const updated = await bookingService.cancelBookingForAdmin(
        booking.id,
        cancelReason,
      );
      setBooking(updated);
  ...
  const handleSaveMileageEnd = async () => {
    ...
    if (
      ...
      Number(mileageEnd) <= booking.mileageStart
    ) {
      toast.error(
        `End mileage must be greater than start mileage (${booking.mileageStart.toLocaleString()} km).`,
      );
      return;
    }
    ...
    if (!showReassign && allCars.length === 0 && booking) {
      ...
      try {
        ...
        const candidates = cars.filter(
          (c) => c.id !== booking.carId && c.isAvailable,
        );
  ...
  const handleReassign = async () => {
    ...
    setReassigning(true);
    try {
      await bookingService.reassignCar(
        booking.id,
  ...
  return (
    <div>
      ...
      {booking.bookingStatus === "PENDING" && (
        <div
          id="action-approve"
          className="bg-white rounded-xl border border-light-gray p-5 shadow-sm mb-4"
        >
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Confirm the customer&apos;s identity and verify payment before
                approving.{" "}
                <Link
        ...
        <div className="flex items-start gap-3 bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 mb-4">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-700">
              Step 1 — Complete pre-inspection before handover
            </p>
            <p className="text-xs text-amber-600 mt-0.5">
              Record the pre-inspection below, then have the customer confirm
              it. Start mileage can only be entered after confirmation.
            </p>
          </div>
        </div>
      )}
      {booking.bookingStatus === "CONFIRMED" &&
        ...
        !booking.mileageStart && (
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-700">
                Step 2 — Awaiting customer confirmation of pre-inspection
              </p>
              <p className="text-xs text-amber-600 mt-0.5">
                The customer must confirm the pre-inspection before you can
                record the start mileage and hand over the vehicle.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-start gap-3 bg-emerald-50 border border-emerald-300 rounded-xl px-4 py-3 mb-4">
            ...
            <div>
              <p className="text-sm font-semibold text-emerald-700">
                Step 3 — Pre-inspection confirmed. Record start mileage to hand
                over.
              </p>
              <p className="text-xs text-emerald-600 mt-0.5">
                Enter the odometer reading below and click &quot;Record
                Start&quot; to complete the handover.
              </p>
            </div>
          </div>
        )}
              ...
              <div className="flex flex-col gap-1">
                ...
                {(!preInspection ||
                  ...
                  <p className="text-xs text-amber-600">
                    Complete &amp; confirm pre-inspection first
                  </p>
                )}
              </div>
            )}
            ...
            <h2 className="font-semibold text-navy text-sm flex items-center gap-2">
              ...
              {extensions.some((e) => e.extensionStatus === "PENDING") && (
                <span className="inline-flex items-center justify-center h-4.5 min-w-4.5 px-1 text-[10px] font-bold rounded-full bg-warning text-white">
                  {
                    extensions.filter((e) => e.extensionStatus === "PENDING")
                      .length
                  }
                </span>
              )}
            ...
            {extensionsLoading ? (
              <div className="px-5 py-4 space-y-2">
                {[0, 1].map((i) => (
                  <div
                    key={i}
                    className="h-16 bg-light-gray rounded-lg animate-pulse"
                  />
                ))}
              </div>
            ) : (
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        >
                          ...
                          {ext.extensionStatus.charAt(0) +
                            ext.extensionStatus.slice(1).toLowerCase()}
                        </span>
                        <span className="text-sm font-semibold text-navy">
                          +{ext.requestedDays} day
                          {ext.requestedDays !== 1 ? "s" : ""}
                        </span>
                        {ext.extensionCost != null && (
                          <span className="text-xs text-muted-foreground">
                            · KES {ext.extensionCost.toLocaleString()}
                          </span>
                        )}
                      </div>
                      <p className="text-[11px] text-muted-foreground font-mono">
                        {ext.extensionReference}
                      </p>
                      {ext.customerNote && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Customer: {ext.customerNote}
                        </p>
                      )}
                      {ext.adminNote && ext.extensionStatus !== "PENDING" && (
                        <p className="text-xs text-royal mt-0.5">
                          Admin note: {ext.adminNote}
                        </p>
                      )}
                    </div>
                  </div>
      ...
      {booking.bookingStatus === "PENDING" && (
        <button
          ...
          className="w-full flex items-center justify-between bg-white rounded-xl border border-light-gray shadow-sm px-5 py-4 mb-4 hover:bg-off-white transition-colors"
        >
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-lg bg-royal/10 border border-royal/20 flex items-center justify-center shrink-0">
              <Pencil className="h-4 w-4 text-royal" />
            </div>
            <div className="text-left">
              <p className="font-semibold text-navy text-sm">Edit Booking</p>
              <p className="text-xs text-muted-foreground">
                Change the pick-up/return dates or destination
              </p>
            </div>
          </div>
      ...
      <EditBookingModal
        ...
        onUpdated={() => {
          bookingService
            .getBookingByIdForAdmin(booking.id)
            .then(setBooking)
            .catch(() => {});
        }}
      />
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="font-semibold text-navy text-sm">
                  Change Car in This Booking
                </p>
                <p className="text-xs text-muted-foreground">
                  Swap the vehicle — dates and destination stay the same
                </p>
              </div>
            </div>
            {showReassign ? (
              ...
              {carsLoading ? (
                <div className="space-y-2">
                  {[0, 1, 2].map((i) => (
                    <div
                      key={i}
                      className="h-14 bg-light-gray rounded-lg animate-pulse"
                    />
                  ))}
                </div>
              ) : allCars.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    All other cars are already booked from{" "}
                    {fmt(booking.startDate)} to {fmt(booking.endDate)}.
                  </p>
                </div>
              ) : (
                <>
                  <p className="text-xs text-muted-foreground mb-3">
                    ...
                    car{allCars.length !== 1 ? "s" : ""} confirmed free from{" "}
                    <span className="font-semibold text-navy">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {c.numberPlate} · KES{" "}
                              {c.pricePerDay.toLocaleString()}/day
                            </p>
                          </div>
                        </div>
      ...
      {/* Cancel Booking a PENDING or CONFIRMED */}
      {(booking.bookingStatus === "PENDING" ||
        booking.bookingStatus === "CONFIRMED") && (
        <div className="bg-danger/5 border border-danger/20 rounded-xl p-5 mt-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                cancel to make the car available for booking
              </p>
            </div>
            <Button
              variant="destructive"
              ...
              disabled={cancelling}
              className="bg-danger hover:bg-danger/90 text-white shrink-0 gap-2"
            >
          ...
          <textarea
            ...
            className="mt-3 w-full rounded-lg border border-light-gray bg-white px-3 py-2 text-sm text-navy placeholder:text-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-danger/30 transition-colors"
          ></textarea>
        </div>
      )}
    </div>
