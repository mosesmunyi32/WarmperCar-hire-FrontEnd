...
import { AdminBooking, AdminCar, AdminInspectionResponse } from "@/types";
import {
  format,
  differenceInDays,
  startOfDay,
...
interface AdminAction {
  ...
  title: string;
  action: string;
  kind: "approve" | "inspection" | "mileage";
}
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([b, c, cu]) => {
        ...
        // Build the "Action Required" feed — items needing the admin's attention.
        const carMap2 = new Map(c.map((car) => [car.id, car]));
        const needInsp = b.filter(
          (bk) =>
        ...
        Promise.allSettled(
          needInsp.map((bk) =>
            inspectionService.getBookingInspectionsForAdmin(bk.id),
          ),
        ).then((results) => {
          const inspMap = new Map<string, AdminInspectionResponse[]>();
          results.forEach((res, i) => {
            if (res.status === "fulfilled")
              inspMap.set(needInsp[i].id, res.value);
          });
          const actions: AdminAction[] = [];
          b.forEach((bk) => {
            const car = carMap2.get(bk.carId);
            const carLabel = car
              ? `${car.brand} ${car.model} · ${car.numberPlate}`
              : bk.bookingReference;
            const insp = inspMap.get(bk.id) ?? [];
            const pre = insp.find((x) => x.inspectionType === "PRE_INSPECTION");
            const post = insp.find(
              (x) => x.inspectionType === "POST_INSPECTION",
            );
            ...
            } else if (bk.bookingStatus === "CONFIRMED") {
              if (!pre) {
                actions.push({
                  bookingId: bk.id,
                  car: carLabel,
                  title: "Record pre-inspection",
                  action: "pre-inspection",
                  kind: "inspection",
                });
              } else if (
                pre.customerResponse === "CONFIRMED" &&
                !bk.mileageStart
              ) {
                actions.push({
                  bookingId: bk.id,
                  car: carLabel,
                  title: "Customer confirmed — record start mileage",
                  action: "mileage-start",
                  kind: "mileage",
                });
              }
            } else if (bk.bookingStatus === "COMPLETED") {
              if (pre?.customerResponse === "CONFIRMED" && !post) {
                actions.push({
                  bookingId: bk.id,
                  car: carLabel,
                  title: "Record post-inspection",
  ...
  // Cars currently handed over (start mileage recorded, not yet returned).
  ...
  return (
    <div>
      ...
      {/* Action Required — items needing the admin's attention */}
      {!loading && adminActions.length > 0 && (
        <div className="rounded-2xl border border-gold/35 bg-gold/[0.06] overflow-hidden mb-6">
          <div className="flex items-center gap-2 px-5 py-3 border-b border-gold/20">
            <ClipboardCheck className="h-4 w-4 text-gold" />
            <h2 className="font-bold text-navy text-sm">Action Required</h2>
            <span className="ml-1 inline-flex items-center justify-center h-5 min-w-5 px-1.5 text-[11px] font-bold rounded-full bg-gold text-navy">
              {adminActions.length}
            </span>
          </div>
          <div className="divide-y divide-gold/15 max-h-80 overflow-y-auto">
            {adminActions.map((a, i) => {
              ...
              return (
                <Link
                  key={`${a.bookingId}-${i}`}
                  href={`/admin/bookings/${a.bookingId}?action=${a.action}`}
                  className="flex items-center gap-3 px-5 py-3 hover:bg-gold/[0.08] transition-colors"
                >
                  <div className="h-9 w-9 rounded-lg bg-gold/15 border border-gold/25 flex items-center justify-center shrink-0">
      ...
      {!loading && <TripsOnRoad trips={trips} hrefBase="/admin/bookings" />}
