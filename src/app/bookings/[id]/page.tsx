...
export default function BookingDetailPage() {
    ...
    bookingService
      ...
      .then((b) => {
        ...
        carService
          ...
          .catch(() => {});
        inspectionService
          .getBookingInspectionsForCustomer(b.id)
          .then(setInspections)
          .catch(() => {});
        bookingService
          .getMyExtensions()
          .then((all) => setExtensions(all.filter((e) => e.bookingId === b.id)))
          .catch(() => {});
  ...
  const handleRespond = async (
    inspectionId: string,
    data: InspectionRespondRequest,
  ) => {
    setRespondingId(inspectionId);
    try {
      const updated = await inspectionService.respondToInspection(
        inspectionId,
        data,
      );
      setInspections((prev) =>
      ...
      toast.success(
        data.customerResponse === "CONFIRMED"
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      toast.success(
        `booking switched to ${selectedCar.brand} ${selectedCar.model}`,
      );
  ...
  const handleRequestExtension = async () => {
    ...
    try {
      ...
      setExtNote("");
      toast.success(
        "Extension request submitted. We'll notify you once it's reviewed.",
      );
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { message?: string } } })
        ?.response?.data?.message;
          ...
          <>
              ...
              <div className="bg-gold/10 border border-gold/30 rounded-xl p-4 mb-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground mt-1">
                    ...
                    Your booking will be confirmed once payment is received and
                    verified by our team.
                  </p>

                  {booking.expiresAt && (
                    <p className="text-sm font-semibold text-red-500 mt-2 items-center gap-1.5">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="text-right flex flex-col items-end gap-2">
                  <p className="text-2xl font-bold text-royal">
                    KES {(booking.bookingCost ?? 0).toLocaleString()}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {booking.numberOfDays} day
                    {booking.numberOfDays !== 1 ? "s" : ""} × KES{" "}
                  ...
                  {booking.receiptUrl ? (
                    ...
                    >
                      <RefreshCw
                        className={`h-3 w-3 ${checkingReceipt ? "animate-spin" : ""}`}
                      />
                      {checkingReceipt
                        ? "Checking…"
                        : "Receipt not yet generated — tap to check"}
                    </button>
                  ) : null}
                </div>
              </div>
                      ...
                      value ? (
                        <div key={label} className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs font-semibold text-navy truncate capitalize">
                              {value.toLowerCase()}
                            </p>
                          </div>
                        </div>
              ...
              <div className="bg-white rounded-xl border border-light-gray p-5 shadow-sm">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-navy">
                      KES {booking.pricePerDay.toLocaleString()} /day
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Duration</span>
                  ...
                  {booking.discount > 0 && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-success">
                        - KES {booking.discount.toLocaleString()}
                      </span>
                    </div>
                  )}
            ...
            {booking.bookingStatus === "PENDING" && (
              <button
                ...
                className="w-full flex items-center justify-between bg-white rounded-xl border border-light-gray mb-4 shadow-sm px-5 py-4 hover:bg-off-white transition-colors"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Adjust your dates or destination before it&apos;s confirmed
                    </p>
                  </div>
                </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Switch to another car — your dates and destination stay
                        the same
                      </p>
                    </div>
                  </div>
                    ...
                    {swapLoading ? (
                      ...
                      <div className="text-center py-6">
                        ...
                        <p className="text-sm font-medium text-navy">
                          No other cars available for your dates
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">
                          All other vehicles are booked from{" "}
                      ...
                      <>
                        <p className="text-xs text-muted-foreground mb-3">
                          ...
                          car{swapCars.length !== 1 ? "s" : ""} available from{" "}
                          <span className="font-semibold text-navy">
                            {formatDate(booking.startDate)}
                          </span>{" "}
                          to{" "}
              ...
              <div className="bg-white rounded-xl border border-light-gray mb-4 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-4 border-b border-light-gray">
                  <h2 className="font-semibold text-navy text-sm flex items-center gap-2">
                    <CalendarPlus className="h-4 w-4 text-royal" /> Booking
                    Extensions
                  </h2>
                  {booking.bookingStatus === "CONFIRMED" &&
                    ...
                    !showExtensionForm && (
                      ...
                      >
                        <CalendarPlus className="h-3.5 w-3.5" /> Request
                        Extension
                      </button>
                    )}
                </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            disabled={
                              submittingExt || !extDays || Number(extDays) < 1
                            }
                            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-royal text-white text-sm font-semibold hover:bg-navy transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          >
